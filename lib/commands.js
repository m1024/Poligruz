...
commands.execCommand = function (command, socket) {
    ...
    if (typeof commands.list[command.name] === 'function') {
        ...
        if (sessionID === undefined){
            //Логирование события
            log.warn('Session undefined: ' + command.name + '. From: ' + socket.id);
            socket.emit('delete_cookie', 'connect.sid'); //на клиенте нет доступа к этим cookie, но хотя бы попросим пользователя почистить вручную (и узнаем у кого именно падает)
            // socket.handshake.session.reload(function(err) {
            //     log.warn('Session reload, err: ' + err);
            // });
            return;
        }
