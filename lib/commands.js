...
commands.getSessionIDFromSocket = function (socket) {
    var values = cookie.parse(socket.handshake.headers.cookie);
    if (values['connect.sid'] === undefined) return undefined;
    var sessionID = values['connect.sid'].substring(0, values['connect.sid'].indexOf('.')).substring(2);
    return sessionID;
    // if (sessionID === undefined)
    //     throw 'SessionID not found';
    // else
    //     return sessionID;
};
